...
### [1] [A numerical study on the effect of rolling friction on clogging of pores in particle-laden flows](https://arxiv.org/abs/2601.11121)
*Sagar G. Nayak,Zhenjiang You,Yuchen Dai,Geoff Wang,Prapanch Nair*

Main category: physics.flu-dyn

TL;DR: 该论文通过直接数值模拟研究了滚动阻力对多孔介质中致密悬浮液堵塞行为的影响，开发了DEM-IBM耦合方法进行孔隙尺度模拟。


<details>
  <summary>Details</summary>
Motivation: 研究滚动阻力对多孔介质中悬浮液堵塞行为的影响，因为现有文献对滚动阻力系数作为综合参数（表征颗粒刚性、形状、润滑和流体阻力）对堵塞行为的直接影响研究不足。

Method: 开发了离散元法（DEM）库并与基于浸没边界法（IBM）的开源求解器耦合，进行孔隙和颗粒分辨的直接数值模拟（DNS），通过3D验证DEM库和DEM-IBM耦合方法。

Result: 论文展示了DEM库和DEM-IBM耦合的多个3D验证结果，并研究了滚动阻力对孔隙入口处堵塞行为的影响。

Conclusion: 通过孔隙尺度的直接数值模拟，系统研究了滚动阻力对致密悬浮液在多孔介质中堵塞行为的影响，为理解颗粒接触力学在孔隙堵塞中的作用提供了新见解。
...
### [2] [Scale-resolving simulations and data-driven modal analysis of turbulent transonic buffet cells on infinite swept wings](https://arxiv.org/abs/2601.11137)
*David J. Lusher,Andrea Sansica*

Main category: physics.flu-dyn

TL;DR: 该研究通过隐式大涡模拟和模态分析，首次在展弦比达3的无限翼展机翼上研究了后掠效应对跨音速翼型抖振的影响，揭示了抖振由二维激波运动和三维分离驱动的不稳定性叠加而成。


<details>
  <summary>Details</summary>
Motivation: 由于计算成本限制，以往跨音速翼型抖振的尺度解析模拟仅限于窄机翼，无法充分研究实验和低精度模拟中报告的三维抖振单元不稳定性，需要研究更大展弦比和后掠效应下的三维抖振特性。

Method: 采用隐式大涡模拟(ILES)和模态分析方法，在无限翼展机翼上（展弦比达3）首次研究后掠效应，分析两种流动条件：激波位置处最小分离和大幅分离的平均流动。

Result: 最小分离情况下激波动力学基本保持展向均匀（准二维），而增加平均分离导致显著三维抖振单元出现（特征展向波长1-1.5c）。后掠使先前在无后掠机翼上识别的低频三维分离模式转变为展向传播模式，频率向中频移动，而二维激波模式对后掠不敏感。

Conclusion: 跨音速抖振源于二维激波运动和分离驱动的三维不稳定性的叠加，激波处的平均流动分离是主导三维抖振动力学出现的必要条件，后掠增强了三维模式的频率和能量含量但不改变其波长。
...
### [3] [Inertial Self-Caging: Dynamics of Macroscopic Swimmers at Moderate Reynolds Number Sustaining Chemical Wake Resonance](https://arxiv.org/abs/2601.11264)
*Alessandro Foradori,Paolo Bettotti*

Main category: physics.flu-dyn

TL;DR: 研究展示了在中等雷诺数（100<Re<200）惯性流态下，化学驱动的水凝胶游泳者通过化学自反馈机制产生稳定共振运动，并存在从共振振荡到随机停走行为的临界转变。


<details>
  <summary>Details</summary>
Motivation: 传统自推进泳体研究主要关注层流低雷诺数区域，本研究旨在探索惯性流态（中等雷诺数）下化学驱动游泳者的推进机制，该区域流体动力学呈现非线性特性。

Method: 使用化学驱动的宏观水凝胶游泳者，在受限几何条件下进行实验研究，并结合建模分析，研究游泳者与自身持久化学尾迹的相互作用。

Result: 发现了稳定共振态的存在：游泳者作圆周运动时，与自身化学尾迹相互作用产生化学自反馈，形成复杂的稳定运动，表现为普遍指数速度衰减叠加显著周期性速度振荡。识别了临界阈值速度，系统在此处从共振振荡态突然转变为随机停走行为。

Conclusion: 研究揭示了化学场、流体惯性力和几何限制如何耦合决定高速活性物质的运动特性，为理解惯性流态下化学驱动游泳者的动力学行为提供了基础性认识。
...
### [4] [Analytical Solutions of the Minimal Nonlinear Equation for the Yaw Response of Tail Fins and Wind Vanes](https://arxiv.org/abs/2601.11349)
*Mohamed M. Hammam,David H. Wood*

Main category: physics.flu-dyn

TL;DR: 该研究推导了小型风力机尾翼和风向标偏航响应的解析解，适用于任意翼型和初始释放角，扩展了现有线性模型的局限性。


<details>
  <summary>Details</summary>
Motivation: 现有线性模型仅适用于小初始角度和低展弦比翼型，无法准确描述尾翼和风向标的非线性偏航响应，需要更通用的解析解。

Method: 基于Hammam和Wood推导的二阶偏航角方程，使用微扰技术分析最小化方程；采用Krylov-Bogoliubov-Mitropolskii平均法和Beecham-Titchener平均法获得近似解析解。

Result: 获得了小角度和大角度两种极限情况的解析解；高涡流系数K_v对尾翼大角度下的快速衰减至关重要；高展弦比可减小风向标的非线性误差；小角度解可用于参数识别和非线性建模扩展。

Conclusion: 该研究提供了适用于任意翼型和初始角的尾翼/风向标偏航响应解析解，揭示了非线性阻尼机制，为小型风力机尾翼设计和风向标精度优化提供了理论依据。
...
### [5] [Differential geometry of particle motion in Stokesian regime](https://arxiv.org/abs/2601.11377)
*Sumedh R. Risbud*

Main category: physics.flu-dyn

TL;DR: 论文提出了一个微分几何框架来描述非布朗粒子在静止流体中绕过固定障碍物的运动，证明物理轨迹不是纯阻力度规的测地线，而是耗散缩放度规的测地线。


<details>
  <summary>Details</summary>
Motivation: 虽然亥姆霍兹最小耗散定理表明流体阻力张量可作为流体域的黎曼度规，但实际观察发现恒定外力驱动的粒子轨迹并非该纯阻力度规的测地线，需要建立统一的几何形式来解释这一现象。

Method: 引入微分几何框架，证明物理轨迹是共形缩放度规 $\tilde{g}_{ij} = \mathcal{D}(\mathbf{x})R_{ij}$ 的测地线，其中 $\mathcal{D}$ 是局部功率耗散，轨迹的仿射参数对应于累积能量耗散。

Result: 建立了统一的几何形式主义，将粒子轨迹解释为耗散缩放流形上的测地线，并应用于球形粒子绕过固定障碍物的散射问题，证明先前推导的轨迹是该流形曲率的直接结果。

Conclusion: 物理轨迹不是纯阻力度规的测地线，而是耗散缩放度规的测地线，这一框架为斯托克斯流中的粒子运动提供了统一的几何解释，将轨迹几何与能量耗散联系起来。
...
### [6] [CTHA: Constrained Temporal Hierarchical Architecture for Stable Multi-Agent LLM Systems](https://arxiv.org/abs/2601.10738)
*Percy Jardine*
...
TL;DR: 论文提出基于趋势启发式的复杂产品创新过程建模方法，使用简单趋势（递增、递减、恒定）作为最小信息强度量化器，通过场景集合和转移图表示系统行为


<details>
  <summary>Details</summary>
Motivation: 研究复杂产品创新过程需要有效的建模方法，传统方法依赖数值或粗糙集，信息强度要求高。本文旨在开发一种基于简单趋势启发式的轻量化建模框架，避免对数值数据的依赖

Method: 采用基于启发式的趋势模型，每个启发式通过简单趋势（递增、递减、恒定）表达，作为最小信息强度量化器。定义趋势模型的解为场景集合及其可能的转移关系，用转移图表示。系统任何可能的未来或过去行为都可通过图中的路径描绘

Result: 建立了基于趋势启发式的复杂产品创新过程建模框架，能够用简单趋势量化系统行为，通过转移图可视化所有可能的系统演化路径，提供了一种不依赖数值数据的轻量化分析方法

Conclusion: 趋势启发式模型为复杂产品创新过程分析提供了有效的轻量化框架，避免了传统数值方法的复杂性，通过简单趋势和转移图能够全面描述系统的可能行为演化路径
...
TL;DR: ARC-AGI-2竞赛显示AI在抽象推理方面进展有限，最高得分仅24%，但涌现出基于反馈的精细化循环方法，前沿AI实验室开始将其作为行业标准基准。


<details>
  <summary>Details</summary>
Motivation: ARC-AGI基准系列是衡量AI在少量样本下泛化到新任务能力的关键指标，对于评估流体智能和抽象推理能力至关重要。2025年竞赛旨在推动AI在复杂推理任务上的进步，并揭示当前方法的局限性。

Method: 论文分析了2025年ARC-AGI-2竞赛中的多种方法，重点关注精细化循环（refinement loop）技术，包括进化程序合成方法和商业AI系统的应用层优化。同时考察了零预训练深度学习方法和前沿AI实验室的模型卡报告。

Result: 竞赛最高得分仅为24%，显示AI在抽象推理方面仍有很大提升空间。精细化循环方法成为主流，前沿AI实验室开始将ARC-AGI作为行业标准基准。研究发现当前AI推理性能仍受限于知识覆盖，存在新的基准污染形式。

Conclusion: ARC-AGI已成为AI推理能力的行业标准基准，但当前方法仍主要依赖知识覆盖而非真正的推理能力。未来需要开发能够进行探索、规划、记忆和目标获取的交互式推理系统，ARC-AGI-3将引入这些挑战。
...
TL;DR: 该论文研究了多模态推理的数据筛选，通过NeurIPS 2025 DCVLR挑战赛发现：在固定模型和训练协议下，基于难度的样本选择是性能提升的主要驱动力，而增加数据集大小主要降低方差而非提升平均精度，多样性和合成增强启发式方法无额外益处。


<details>
  <summary>Details</summary>
Motivation: 研究多模态推理中的数据筛选问题，通过DCVLR挑战赛（固定模型和训练协议，仅改变数据集）来隔离数据集选择的影响，探索在数据高效多模态推理中数据筛选的关键因素。

Method: 使用基于Walton多模态冷启动数据集的紧凑筛选数据集参与DCVLR挑战赛，并在赛后进行消融实验，分析难度选择、数据集大小、多样性和合成增强等数据筛选策略对性能的影响。

Result: 1) 基于难度的样本选择是性能提升的主要驱动因素；2) 增加数据集大小主要降低运行间方差而非提升平均精度；3) 常用的多样性和合成增强启发式方法无额外益处且常降低性能；4) 该团队提交在挑战赛中排名第一。

Conclusion: DCVLR是一个饱和状态评估，突出了对齐和难度在数据高效多模态推理中的核心作用。在固定训练方案下，精心筛选的难度样本比单纯增加数据量或使用复杂增强策略更有效。
...
TL;DR: ReCreate：一个基于经验驱动的自动领域智能体创建框架，通过智能体交互历史学习成功/失败原因，实现高效智能体生成


<details>
  <summary>Details</summary>
Motivation: 当前大多数实用智能体仍由人工设计，任务差异大导致构建成本高；现有自动生成方法将智能体创建视为黑盒过程，仅依赖最终性能指标，忽略了成功/失败的关键证据且计算成本高

Method: 提出ReCreate框架，采用智能体即优化器范式，包含三个核心组件：1) 经验存储与检索机制，支持按需检查；2) 推理-创建协同管道，将执行经验映射到脚手架编辑；3) 分层更新机制，将实例级细节抽象为可重用的领域模式
...
TL;DR: SCALE框架通过任务级工作流生成和基于自我预测的评估，在保持性能的同时大幅降低多智能体系统的令牌消耗


<details>
  <summary>Details</summary>
Motivation: 现有多智能体系统工作流生成方法在任务级和查询级各有优劣，但相对成本和效益不明确；查询级生成并非总是必要，而基于执行的任务级评估成本高且不可靠

Method: 提出SCALE框架：任务级工作流生成，通过少量样本校准的自我预测进行优化器评估，替代完整的验证执行

Result: SCALE在多个数据集上保持竞争力，平均性能仅下降0.61%，同时将总体令牌使用量减少高达83%

Conclusion: 任务级工作流生成结合自我预测评估是高效的多智能体系统构建方法，能在显著降低计算成本的同时维持性能
...
TL;DR: TANDEM：一种通过串联强化学习实现音频-视觉仇恨检测的统一框架，将二元分类任务转化为结构化推理问题，显著提升目标识别和时间定位性能


<details>
  <summary>Details</summary>
Motivation: 当前社交媒体上长格式多模态内容日益增多，有害叙事通过音频、视觉和文本线索的复杂交互构建。现有自动化系统虽然能高精度检测仇恨言论，但作为"黑盒"无法提供可解释的细粒度证据（如精确时间戳和目标身份），难以支持有效的人机协同审核。

Method: 提出TANDEM统一框架，采用新颖的串联强化学习策略：视觉-语言和音频-语言模型通过自约束跨模态上下文相互优化，在无需密集帧级监督的情况下稳定处理长时间序列的推理。
...
Conclusion: 即使在复杂的多模态环境中，结构化、可解释的对齐也是可实现的，这为下一代透明且可操作的在线安全审核工具提供了蓝图。TANDEM框架展示了将仇恨检测从二元分类转化为结构化推理问题的价值。
...
<details>
  <summary>Details</summary>
Motivation: 现有评估方法主要关注结果一致性（如准确率、F1分数），缺乏对决策机制的深入理解，无法诊断AI与人类在约束决策中的深层次对齐问题。
...
Conclusion: XChoice提供基于机制的指标，能够诊断不对齐问题并支持超越表面结果匹配的改进，为AI与人类对齐评估提供了更深入的分析工具。
...
### [14] [AstroReason-Bench: Evaluating Unified Agentic Planning across Heterogeneous Space Planning Problems](https://arxiv.org/abs/2601.11354)
*Weiyi Wang,Xinchi Chen,Jingjing Gong,Xuanjing Huang,Xipeng Qiu*

Main category: cs.AI

TL;DR: 论文介绍了AstroReason-Bench基准测试，用于评估大型语言模型在具有严格物理约束的太空规划问题中的智能体规划能力，发现当前智能体性能远低于专用求解器。


<details>
  <summary>Details</summary>
Motivation: 现有智能体基准测试主要关注符号化或弱接地环境，缺乏对物理约束现实世界领域的评估。太空规划问题具有异构目标、严格物理约束和长时程决策等特点，是评估智能体规划能力的理想高风险场景。

Method: 开发了AstroReason-Bench基准测试，整合了地面站通信和敏捷地球观测等多种调度机制，提供了统一的智能体导向交互协议。在多种最先进的开源和闭源智能体LLM系统上进行了评估。

Result: 当前智能体在太空规划问题中表现显著低于专用求解器，揭示了通用规划器在现实约束下的关键局限性。基准测试为未来智能体研究提供了具有挑战性和诊断性的测试平台。

Conclusion: AstroReason-Bench填补了智能体基准测试在物理约束现实世界领域的空白，为评估和改进智能体在复杂、高风险环境中的规划能力提供了重要工具，揭示了当前通用规划方法的局限性。
...
<details>
  <summary>Details</summary>
Motivation: 约束规划求解器的性能高度依赖超参数设置，手动调参困难耗时且需要专业知识，需要自动化方法来优化求解器配置

Method: 提出两阶段框架：1) 探针阶段使用可配置的超参数优化方法（贝叶斯优化和汉明距离搜索）探索不同超参数集；2) 求解阶段使用找到的最佳配置在剩余时间内解决问题。框架集成到CPMpy库中

Result: 在114个组合问题实例上评估，使用贝叶斯优化时：ACE求解器在25.4%实例中性能提升，57.9%实例与默认配置相当；Choco求解器在38.6%实例中表现更优。贝叶斯优化始终优于汉明距离搜索

Conclusion: 探针与求解算法提供了一种实用、资源感知的约束求解器调优方法，通过模型化探索优于简单局部搜索，能在多样化问题类型上实现稳健的性能改进
...
<details>
  <summary>Details</summary>
Motivation: 预测性过程监控是过程挖掘的重要分支，旨在预测进行中过程的结果。虽然已有基于机器学习和深度学习的架构，但作者先前提出的基于LLM的框架仅专注于通过提示进行总时间预测。本文旨在扩展该框架，全面评估其泛化能力、语义利用和推理机制，并扩展到多个关键绩效指标。

Method: 扩展了先前的LLM-based预测性过程监控框架，从单一的总时间预测扩展到多KPI预测。通过实证评估在三个不同事件日志上进行测试，重点关注总时间和活动发生预测两个KPI。特别在数据稀缺设置下（仅100条轨迹）进行实验，分析LLM如何利用其先验知识和训练轨迹间的内部相关性。最后，通过检查模型使用的推理策略，验证LLM是否仅复制现有预测方法。

Result: 在数据稀缺设置（仅100条轨迹）下，LLM在总时间和活动发生预测方面超越了基准方法。实验表明LLM既利用了其内在的先验知识，也利用了训练轨迹间的内部相关性。对推理策略的分析显示，LLM并非简单地复制现有预测方法，而是执行高阶推理来生成预测。

Conclusion: 基于LLM的预测性过程监控框架具有良好的泛化能力，能够在数据稀缺场景下有效预测多个关键绩效指标。LLM不仅利用其先验知识，还能进行高阶推理，这为预测性过程监控提供了新的有效方法，特别是在数据有限的情况下。
...
TL;DR: 提出LEG框架，结合可证明的近似算法与LLM驱动的迭代优化，将专家定性指导融入人口覆盖优化，用于埃塞俄比亚卫生站升级的优先级决策


<details>
  <summary>Details</summary>
Motivation: 埃塞俄比亚卫生部需要升级卫生站以改善基本医疗服务可及性，但资源有限，需要优先考虑哪些设施进行升级以最大化人口覆盖，同时考虑专家和利益相关者的多样化偏好。传统优化方法需要明确的量化目标，而利益相关者的标准通常用自然语言表达，难以形式化。

Method: 提出LEG（大语言模型与扩展贪心）混合框架：1）使用可证明近似算法进行人口覆盖优化；2）结合LLM驱动的迭代优化，通过人机对齐将专家定性指导融入解决方案，同时保持覆盖保证。

Result: 在埃塞俄比亚三个地区的真实数据上进行实验，证明了该框架的有效性，展示了其在促进公平、数据驱动的卫生系统规划方面的潜力。

Conclusion: LEG框架成功地将专家知识与优化技术相结合，解决了卫生系统规划中定量优化与定性专家指导之间的鸿沟，为资源有限环境下的公平卫生服务规划提供了实用工具。
...
TL;DR: BoxMind：一个用于拳击战术分析的闭环AI专家系统，通过定义原子击打事件和分层技术-战术指标，结合图预测模型和可学习潜嵌入，实现比赛结果预测和战术调整，在2024年巴黎奥运会上成功应用。


<details>
  <summary>Details</summary>
Motivation: 拳击等格斗类运动缺乏AI驱动的战术分析，主要由于动作动态复杂且缺乏结构化战术表示。现有方法难以捕捉拳手对战的动态特性，需要将非结构化视频数据转化为战略智能。

Method: 1. 定义具有精确时间边界和空间技术属性的原子击打事件；2. 将比赛视频解析为18个分层技术-战术指标；3. 提出基于图的预测模型，融合显性技术-战术特征和可学习时变潜嵌入；4. 将比赛结果建模为技术-战术指标的可微函数，将获胜概率梯度转化为可执行的战术调整。
...
TL;DR: 该论文是一篇关于协作连续体机器人(CCRs)的综述文章，系统总结了这一新兴领域的研究进展、分类框架、关键技术及未来挑战。


<details>
  <summary>Details</summary>
Motivation: 连续体机器人(CRs)因其紧凑结构、固有柔顺性和灵活变形能力已广泛应用于多个领域。通过协调多个CRs形成协作连续体机器人(CCRs)，可以进一步提升任务适应性、工作空间、灵活性、负载能力和操作稳定性，具有显著优势。近年来该领域兴趣持续增长，相关文献不断增加，需要系统性的综述来梳理研究框架。

Method: 论文采用综述研究方法，首先将CCRs分为三种协作模式：分离协作、辅助协作和并行协作，并给出明确定义。然后从系统架构层面系统总结每种模式在结构设计、建模、运动规划和控制方面的最新进展。最后讨论当前挑战和未来机遇。

Result: 该综述为CCRs研究提供了清晰的框架，系统梳理了三种协作模式下的技术进展，包括：1) 分离协作模式下的多机器人协调；2) 辅助协作模式下的主从式协同；3) 并行协作模式下的刚性连接系统。同时识别了当前研究的关键技术瓶颈和发展方向。

Conclusion: 协作连续体机器人是一个具有重要应用前景的新兴研究领域。通过系统分类和综述分析，论文为研究者提供了清晰的研究框架，指出了结构设计、精确建模、智能规划和控制策略等方面的技术挑战，为未来CCRs的发展指明了方向。
...
TL;DR: ROS 2实时性研究综述：分析、增强和扩展ROS 2以支持实时执行的全面调研


<details>
  <summary>Details</summary>
Motivation: ROS 2作为机器人应用的重要中间件框架，在过去六年中受到实时系统社区和工业界越来越多的关注。然而，需要系统性地了解和分析ROS 2的实时能力，为研究者和实践者提供指导。

Method: 首先详细描述ROS 2的内部调度机制和分层架构，包括与DDS通信和其他通信中间件的交互。然后回顾文献中的关键贡献，涵盖单线程和多线程执行器的时序分析、响应时间、反应时间和数据年龄等指标，以及不同的通信模式。还讨论了社区驱动的ROS 2运行时增强，包括新的执行器算法设计、实时GPU管理和通过micro-ROS的微控制器支持。此外，总结了限制DDS通信延迟、消息过滤器和分析工具的技术。

Result: 提出了基于不同标准对调研贡献进行分类的分类法，系统化了这一不断增长的研究领域。为理解ROS 2的实时能力提供了全面的技术框架和分析方法。

Conclusion: 本调研旨在指导研究者和实践者理解和改进ROS 2的实时能力，为ROS 2在实时机器人应用中的进一步发展提供了系统的知识基础和分类框架。
...
### [21] [Adaptive Sliding Mode Control for Vehicle Platoons with State-Dependent Friction Uncertainty](https://arxiv.org/abs/2601.10724)
*Rishabh Dev Yadav*
...
<details>
  <summary>Details</summary>
Motivation: 多机器人编队控制在车辆编队、有效载荷运输和监视等领域有广泛应用。主要挑战在于处理车轮与地面之间的未知/不确定摩擦力，这些摩擦力随路面条件、轮胎磨损和车速变化而变化。现有自适应控制器难以准确建模和识别这些状态依赖且无法先验有界的摩擦力。

Method: 提出新的自适应滑模控制器，采用两阶段过程：首先，运动学控制器根据期望轨迹计算期望速度；其次，动力学模型生成实现期望运动的指令。通过分离机器人的运动学和动力学，简化控制问题，实现对轮式移动机器人更高效和鲁棒的控制。

Result: 控制器能够处理未知复杂的摩擦力行为，无需先验知识其参数和结构。能够调节编队速度并保持预定义的机器人间距离，即使在存在外部干扰和不确定系统参数的情况下也能实现。

Conclusion: 该自适应滑模控制器为轮式移动机器人车辆编队提供了一种有效的解决方案，能够处理复杂的摩擦力问题，提高编队控制的鲁棒性和性能。
...
### [22] [Multi-Agent Formation Navigation Using Diffusion-Based Trajectory Generation](https://arxiv.org/abs/2601.10725)
*Hieu Do Quang,Chien Truong-Quoc,Quoc Van Tran*

Main category: cs.RO

TL;DR: 提出基于扩散模型的领导者-跟随者编队控制规划器，用于杂乱环境中的多智能体编队运动


<details>
  <summary>Details</summary>
Motivation: 解决杂乱环境中多智能体编队控制的运动规划问题，需要生成平滑轨迹并保持编队几何形状

Method: 使用扩散策略生成两个领导者中点轨迹（视为平面刚性杆），跟随者基于局部坐标相对位置的距离约束编队控制器跟踪领导者

Result: 方法产生平滑运动且跟踪误差低，失败主要发生在狭窄无障碍空间或训练数据集中未包含的障碍配置中

Conclusion: 扩散模型在多智能体编队规划中具有可靠潜力，仿真结果验证了其有效性
...
### [23] [Bidirectional Human-Robot Communication for Physical Human-Robot Interaction](https://arxiv.org/abs/2601.10796)
*Junxiang Wang,Cindy Wang,Rana Soltani Zarrin,Zackory Erickson*

Main category: cs.RO

TL;DR: BRIDGE系统通过自然语言实现人机双向通信，允许用户实时修改机器人轨迹（位置、速度、力），并使用LLM解释用户指令，同时提供语音反馈确保交互透明性。


<details>
  <summary>Details</summary>
Motivation: 物理人机交互需要既适应用户偏好又保持行动透明性的系统。现有系统缺乏允许用户实时修改机器人轨迹并提供双向反馈的自然语言交互能力。

Method: 使用大型语言模型（LLM）在计划运动轨迹和对话历史的上下文中解释用户指令隐含的轨迹修改（位置、速度、力）。系统提供语音反馈，确认修改结果或提出澄清问题。

Result: 18名老年人的用户研究表明，参与者成功使用系统实时修改轨迹。与无语音反馈的消融实验和基线相比，双向反馈显著提高了交互性和透明度的评分，证明机器人语音反馈对更直观的用户体验至关重要。

Conclusion: BRIDGE系统通过自然语言实现双向人机通信，允许实时轨迹修改并提供语音反馈，显著提升了物理辅助任务中的交互性和透明度，为更直观的人机交互体验提供了有效解决方案。
...
### [24] [Approximately Optimal Global Planning for Contact-Rich SE(2) Manipulation on a Graph of Reachable Sets](https://arxiv.org/abs/2601.10827)
*Simin Liu,Tong Zhao,Bernhard Paus Graesdal,Peter Werner,Jiuguang Wang,John Dolan,Changliu Liu,Tao Pang*

Main category: cs.RO

TL;DR: 提出一种新的接触丰富操作规划方法，通过离线构建可达集图和在线规划，实现全局优化的接触丰富操作，显著提升效率和成功率。


<details>
  <summary>Details</summary>
Motivation: 接触丰富操作（CRM）比仅使用末端执行器更高效自然，但现有基于模型的规划器仍侧重于可行性而非最优性，限制了CRM优势的充分发挥。

Method: 采用两阶段方法：离线构建互达集图（包含从起始物体姿态和抓握可达的所有物体方向）；在线在该图上规划，计算并排序局部计划以实现全局优化运动。

Result: 在具有挑战性的接触丰富任务中，相比领先规划器任务成本降低61%；250次查询中成功率91%；查询时间保持在分钟以下。
...
### [25] [Is open robotics innovation a threat to international peace and security?](https://arxiv.org/abs/2601.10877)
*Ludovic Righetti,Vincent Boulanin*

Main category: cs.RO

TL;DR: 该论文探讨机器人学开放获取带来的双重用途风险，提出机器人学社区需要制定特定领域指导原则和监管框架的路线图。


<details>
  <summary>Details</summary>
Motivation: 机器人学领域的开放获取虽然促进了科学进步和系统开发，但也加剧了双重用途风险，使国家和非国家行为者更容易将机器人技术用于军事和有害目的。与其他工程领域相比，机器人学缺乏具体的监管和负责任研究指导。

Method: 提出一个包含四个实践领域的路线图：a) 负责任机器人学教育；b) 激励风险评估；c) 调节高风险材料的传播；d) 制定红线标准。

Result: 论文提出了机器人学社区应对双重用途风险的具体框架，强调需要制定适合机器人学特点的行业特定指导原则和可能的监管措施。

Conclusion: 机器人学社区应当主动制定适合本领域特点的负责任研究指导原则和监管框架，通过教育、风险评估、材料传播控制和红线制定来应对开放获取带来的双重用途风险挑战。
...
### [26] [Where to Touch, How to Contact: Hierarchical RL-MPC Framework for Geometry-Aware Long-Horizon Dexterous Manipulation](https://arxiv.org/abs/2601.10930)
...
<details>
  <summary>Details</summary>
Motivation: 端到端视觉运动策略需要大量数据、仿真到现实迁移差、跨任务泛化弱，而灵巧操作本质上是分层的，需要联合推理几何、运动学约束和非光滑接触动力学

Method: 分层RL-MPC框架：高层RL策略预测接触意图（物体表面接触位置和接触后物体级子目标姿态），底层接触隐式模型预测控制优化局部接触模式并基于接触动力学重新规划

Result: 在非抓取任务（几何泛化推动和物体3D重定向）上实现接近100%成功率，数据需求减少10倍（相比端到端基线），高度鲁棒性能，零样本仿真到现实迁移

Conclusion: 通过将灵巧操作分解为高层几何/运动学规划和底层接触动力学执行的分层框架，解决了端到端方法的局限性，实现了高效、鲁棒、可迁移的灵巧操作
...
### [27] [Crane Lowering Guidance Using a Attachable Camera Module for Driver Vision Support](https://arxiv.org/abs/2601.11026)
*HyoJae Kang,SunWoo Ahn,InGyu Choi,GeonYeong Go,KunWoo Son,Min-Sung Kang*

Main category: cs.RO

TL;DR: 开发用于起重机吊装作业的可附着摄像头模块，通过实时图像传输解决操作员视线被遮挡问题，提高施工现场安全性


<details>
  <summary>Details</summary>
Motivation: 起重机在吊装作业的下降阶段，负载会遮挡操作员对落点的视线，传统依赖地面人员口头或手势指令的方式存在安全隐患，需要技术解决方案

Method: 设计可附着摄像头模块，通过吸盘直接安装在负载上，包含单板计算机、电池和紧凑型摄像头，实时采集和处理负载正下方地面图像，生成安装指导并传输到主机监控

Result: 初步实验通过将模块附着到测试物体上，证实了实时图像采集和传输的可行性，验证了系统基本功能

Conclusion: 该系统有潜力通过为起重机操作员提供隐藏落点的即时视觉参考，显著提高施工现场安全性，解决了传统依赖人工指令的安全隐患
...
### [28] [H-AIM: Orchestrating LLMs, PDDL, and Behavior Trees for Hierarchical Multi-Robot Planning](https://arxiv.org/abs/2601.11063)
...
Method: 提出H-AIM三阶段级联架构：1) 利用LLM解析指令并生成PDDL问题描述；2) 结合LLM语义推理和经典规划器搜索能力生成优化动作序列；3) 将规划结果编译为行为树进行反应式控制。通过共享黑板机制支持动态规模的异构机器人团队通信和状态同步。

Result: 在MACE-THOR基准数据集（8个不同家庭布局中的42个复杂任务）上验证，H-AIM将任务成功率从12%提升到55%，目标条件召回率从32%提升到72%，相比最强基线LaMMA-P有显著改进。
...
### [29] [A3D: Adaptive Affordance Assembly with Dual-Arm Manipulation](https://arxiv.org/abs/2601.11076)
*Jiaqi Liang,Yue Chen,Qize Yu,Yan Shen,Haipeng Zhang,Hao Dong,Ruihai Wu*

Main category: cs.RO

TL;DR: A3D框架通过自适应affordance学习为机器人双臂家具装配提供最优支撑点，利用稠密点云几何表示实现跨几何泛化，并通过交互反馈动态调整支撑策略
...
Method: 提出A3D框架，学习自适应affordance来识别家具零件上的最优支撑和稳定位置。方法采用稠密点级几何表示来建模零件交互模式，实现跨几何泛化。引入自适应模块，利用交互反馈在装配过程中基于先前交互动态调整支撑策略

Result: 建立了包含8种家具类型、50个不同零件的仿真环境，用于双臂协作评估。实验表明，该框架在仿真和真实世界环境中都能有效泛化到不同的零件几何形状和家具类别

Conclusion: A3D框架通过自适应affordance学习和交互反馈机制，成功解决了机器人双臂家具装配中的支撑策略适应和几何泛化问题，在多样化的家具装配任务中表现出色
...
### [30] [VLAgents: A Policy Server for Efficient VLA Inference](https://arxiv.org/abs/2601.11250)
*Tobias Jülg,Khaled Gamal,Nisarga Nilavadi,Pierre Krack,Seongjin Bien,Michael Krawez,Florian Walter,Wolfram Burgard*

Main category: cs.RO

TL;DR: VLAgents是一个模块化的策略服务器，通过统一的Gymnasium风格协议抽象视觉-语言-动作模型推理，支持零拷贝共享内存和压缩流式传输，在本地和远程通信基准测试中优于现有策略服务器。


<details>
  <summary>Details</summary>
Motivation: 视觉-语言-动作模型在机器人领域快速发展，但其部署复杂，存在接口碎片化和分布式设置中的通信延迟问题。

Method: 引入VLAgents模块化策略服务器，采用统一的Gymnasium风格协议抽象VLA推理，通信层透明适配上下文，支持零拷贝共享内存（高速仿真）和压缩流式传输（远程硬件）。

Result: 集成了七个策略（包括OpenVLA和Pi Zero），在本地和远程通信基准测试中，性能优于OpenVLA、OpenPi和LeRobot的默认策略服务器。

Conclusion: VLAgents通过统一的协议和自适应通信层有效解决了VLA模型部署的复杂性和延迟问题，为机器人应用提供了高效可靠的策略服务器解决方案。
...
### [31] [Distributed Control Barrier Functions for Safe Multi-Vehicle Navigation in Heterogeneous USV Fleets](https://arxiv.org/abs/2601.11335)
*Tyler Paine,Brendan Long,Jeremy Wenger,Michael DeFilippo,James Usevitch,Michael Benjamin*

Main category: cs.RO

TL;DR: 该论文提出了一种用于异构无人船队碰撞避免的分布式安全控制滤波器方法，结合控制屏障函数理论和COLREGS规则，在模拟和真实实验中验证了其有效性。


<details>
  <summary>Details</summary>
Motivation: 异构无人船队碰撞避免面临挑战：不同平台的决策过程和控制器存在差异，实时共享轨迹和控制值受限。需要一种能够处理不同平台间差异并能应对人类操作员不合作行为的实用方法。

Method: 采用分布式安全控制滤波器方法，在每个自主车辆上添加控制滤波器，假设其他接触物（包括有人船）采取最坏情况行为。基于控制屏障函数理论开发安全控制滤波器，并与COLREGS行为规则相结合。通过模拟遭遇测试和真实世界实验验证方法。

Result: 模拟实验比较了最坏情况CBF方法和COLREGS行为方法。真实实验使用三种不同的无人船和人类操作船进行，验证了方法在不同平台间的有效性和对人类操作员不合作行为的鲁棒性。结果显示，结合CBF方法和COLREGS行为能实现最佳的安全性和效率。

Conclusion: 结合控制屏障函数理论和COLREGS行为规则的分布式安全控制滤波器方法，能够有效解决异构无人船队的碰撞避免问题，在不同平台间具有良好适应性，并能应对人类操作员的不合作行为，实现安全与效率的最佳平衡。
...
### [32] [ACoT-VLA: Action Chain-of-Thought for Vision-Language-Action Models](https://arxiv.org/abs/2601.11404)
*Linqing Zhong,Yi Liu,Yifei Wei,Ziyu Xiong,Maoqing Yao,Si Liu,Guanghui Ren*

Main category: cs.RO

TL;DR: 提出ACoT-VLA架构，通过动作链式思维范式，在动作空间直接进行推理，提升机器人操作任务的性能。


<details>
  <summary>Details</summary>
Motivation: 现有VLA模型通常依赖多模态输入直接转换为动作，或通过语言子任务预测、视觉目标图像合成等间接推理方式。但这些中间推理往往间接且无法传递精确动作执行所需的完整细粒度信息。作者认为最有效的推理形式是在动作空间直接进行思考。

Method: 提出ACoT-VLA架构，包含两个互补组件：显式动作推理器（EAR）和隐式动作推理器（IAR）。EAR提出粗粒度参考轨迹作为显式动作级推理步骤，IAR从多模态输入的内部表示中提取潜在动作先验，共同形成动作链式思维（ACoT），为下游动作头提供条件，实现接地气的策略学习。

Result: 在真实世界和仿真环境中的大量实验表明，该方法在LIBERO上达到98.5%，LIBERO-Plus上达到84.1%，VLABench上达到47.4%的性能。

Conclusion: ACoT范式通过在动作空间直接进行结构化推理，能够更有效地指导最终策略，显著提升VLA模型在多样化操作任务中的性能。
...
### [33] [The Great March 100: 100 Detail-oriented Tasks for Evaluating Embodied AI Agents](https://arxiv.org/abs/2601.11421)
...
Method: 通过系统分析和扩展现有任务设计，结合人-物交互基元和物体可供性理论，开发了100个任务；在不同机器人平台上收集大量轨迹数据，并评估多个基线模型。
...
### [34] [Learning Semantic-Geometric Task Graph-Representations from Human Demonstrations](https://arxiv.org/abs/2601.11460)
*Franziska Herbert,Vignesh Prasad,Han Liu,Dorothea Koert,Georgia Chalvatzaki*

Main category: cs.RO

TL;DR: 提出语义-几何任务图表示法，从人类演示中学习结构化任务表示，结合MPNN编码器和Transformer解码器，支持双手机器人操作的长时程推理


<details>
  <summary>Details</summary>
Motivation: 从人类演示中学习结构化任务表示对于理解长时程操作行为至关重要，特别是在双手机器人操作中，动作顺序、物体参与和交互几何存在显著变化。主要挑战在于联合捕获任务的离散语义结构和物体中心几何关系的时间演化，以支持对任务进展的推理。

Method: 提出语义-几何任务图表示法，编码物体身份、物体间关系及其时间几何演化。采用学习框架结合MPNN编码器和Transformer解码器，将场景表示学习与动作条件化推理解耦。编码器仅使用时序场景图学习结构化表示，解码器基于动作上下文预测未来动作序列、相关物体和物体运动。

Result: 在人类演示数据集上的广泛评估表明，语义-几何任务图表示特别适用于具有高动作和物体变异性的任务，而简单的基于序列的模型难以捕获任务进展。任务图表示可转移到物理双手机器人并用于在线动作选择。

Conclusion: 语义-几何任务图表示作为可重用的任务抽象，在下游决策系统中具有潜力，能够有效支持双手机器人操作中的长时程推理和动作规划。
...
TL;DR: 提出三种全局平滑、定义在整个实数域且具有解析逆的标量双射函数族，以及一种新颖的径向流架构，在保持训练稳定性的同时显著减少参数数量。


<details>
  <summary>Details</summary>
Motivation: 现有归一化流设计面临表达性与可逆性之间的权衡：仿射变换平滑且可解析求逆但表达能力有限；单调样条提供局部控制但仅分段平滑且定义在有界域；残差流平滑但需要数值求逆。需要结合所有方法优点的解决方案。

Method: 引入三种解析双射函数族：立方有理函数、双曲正弦函数和立方多项式，它们全局平滑(C^∞)、定义在整个实数域且具有闭式解析逆。此外开发径向流架构，通过直接参数化径向坐标变换同时保持角度方向不变。

Result: 在耦合流中作为即插即用替换，匹配或超越样条性能。径向流表现出卓越的训练稳定性，产生几何可解释的变换，对于具有径向结构的目标，能够以1000倍更少的参数实现与耦合流相当的质量。在φ^4晶格场理论的高维物理问题中，新双射函数优于仿射基线并解决模式崩溃问题。

Conclusion: 提出的解析双射函数族结合了现有方法的优点，而径向流架构提供了参数高效且几何可解释的替代方案，为归一化流设计提供了新的灵活性和性能提升。
...
TL;DR: 提出UOWQ框架，统一优化多源迁移学习中的源权重和迁移数量，证明正确调整权重后使用全部源样本总是最优的，并提供理论和实践验证。


<details>
  <summary>Details</summary>
Motivation: 传统多源迁移学习存在负迁移风险，且现有方法通常单独优化源权重或迁移数量，缺乏两者的联合考虑，需要统一的优化框架。

Method: 提出UOWQ理论框架，基于KL散度泛化误差度量的渐近分析，将多源迁移学习建模为参数估计问题；证明权重调整后使用全部源样本最优；为单源提供闭式解，为多源提供凸优化数值方法；进一步提出多源迁移学习和多任务学习的实用算法。

Result: 在DomainNet和Office-Home等真实基准测试中，UOWQ始终优于强基线方法，验证了理论预测和框架的实际有效性。

Conclusion: UOWQ框架通过联合优化源权重和迁移数量，有效解决了多源迁移学习中的负迁移问题，为迁移学习提供了统一的理论和实践指导。
...
TL;DR: 本文提出Mugi架构，通过值级并行性(VLP)优化大语言模型中的非线性操作和小批量GEMM，显著提升吞吐量、能效和可持续性。


<details>
  <summary>Details</summary>
Motivation: 现有VLP主要针对大批量、低精度的对称激活-权重GEMM，但Transformer大语言模型包含更复杂的非线性操作和小批量GEMM，需要探索VLP如何更全面地优化LLM。

Method: 1. 将VLP推广到非线性近似，采用以值为中心的方法，为重要值分配更高精度；2. 优化VLP用于非对称输入的小批量GEMM，结合权重量化、KV缓存量化和分组查询注意力等LLM优化技术；3. 设计新的VLP架构Mugi，集成上述创新并支持完整LLM工作负载。

Result: Mugi在非线性softmax操作上实现吞吐量提升45倍、能效提升668倍；在LLM上实现吞吐量提升2.07倍、能效提升3.11倍；同时降低LLM运行碳排放1.45倍和隐含碳排放1.48倍。

Conclusion: VLP可有效优化LLM中的非线性操作和小批量GEMM，Mugi架构通过集成多项创新技术，在性能、能效和可持续性方面为LLM提供显著改进。
...
### [38] [AI-Guided Human-In-the-Loop Inverse Design of High Performance Engineering Structures](https://arxiv.org/abs/2601.10859)
*Dat Quoc Ha,Md Ferdous Alam,Markus J. Buehler,Faez Ahmed,Josephine V. Carstensen*

Main category: cs.LG

TL;DR: 提出一种结合AI协同设计的拓扑优化方法，通过机器学习预测用户偏好的修改区域，减少人工迭代次数，提高设计效率


<details>
  <summary>Details</summary>
Motivation: 传统拓扑优化计算耗时且黑盒特性阻碍用户交互，现有的人机协同方法依赖耗时的迭代区域选择，需要减少迭代次数

Method: 开发AI协同设计系统，采用U-Net架构的图像分割模型，在合成数据集上训练预测用户偏好区域（最长拓扑构件或最复杂结构连接）

Result: 模型成功预测合理的修改区域，能泛化到多样非标准拓扑优化问题，在训练数据外的单区域选择中表现出涌现行为

Conclusion: 集成AI协同设计的人机协同拓扑优化方法可改善可制造性，将线性屈曲载荷提高39%，总设计时间仅增加15秒
...
### [39] [Beyond Accuracy: A Stability-Aware Metric for Multi-Horizon Forecasting](https://arxiv.org/abs/2601.10863)
*Chutian Ma,Grigorii Pomazkin,Giacinto Paolo Saggese,Paul Smith*

Main category: cs.LG

TL;DR: 提出预测准确性与一致性评分（forecast AC score），用于衡量概率性多步预测的质量，同时考虑多步准确性和稳定性，并通过季节性ARIMA模型在M4 Hourly数据集上验证其有效性。


<details>
  <summary>Details</summary>
Motivation: 传统时间序列预测方法仅优化准确性，忽略了时间一致性（即模型在不同预测起点对同一未来事件预测的一致性）。这种单一目标无法满足实际应用中同时需要准确和稳定预测的需求。

Method: 提出预测准确性与一致性评分（forecast AC score），该评分可衡量概率性多步预测的质量，同时考虑多步准确性和稳定性。评分允许用户指定权重来平衡准确性和一致性要求。作为应用示例，将评分实现为可微分目标函数，用于训练季节性ARIMA模型。

Result: 在M4 Hourly基准数据集上评估，结果显示相比传统最大似然估计有显著改进。AC优化模型在保持相当或改进的点预测准确性的同时，对相同目标时间戳实现了75%的预测波动性降低。

Conclusion: 提出的forecast AC score能够有效平衡预测准确性和时间一致性，为概率性多步预测提供了更全面的质量评估框架，并在实际应用中展示了显著优势。
...
<details>
  <summary>Details</summary>
Motivation: 在离线强化学习和基于模型的强化学习中，世界模型的质量对系统效能和可解释性至关重要，而训练数据质量直接影响世界模型性能。传统训练数据选择方法缺乏系统性和公平性，且Shapley值计算存在指数复杂度问题。

Method: 提出Action Shapley作为与模型无关的训练数据选择度量；设计随机动态算法来降低传统Shapley值计算的指数复杂度；在五个数据受限的实际案例研究中验证方法。

Result: 随机动态算法相比传统指数时间计算实现了超过80%的计算效率提升；基于Action Shapley的训练数据选择策略在性能上持续优于临时性选择方法。

Conclusion: Action Shapley为训练数据选择提供了公平有效的度量标准，其高效计算算法解决了传统方法的复杂度瓶颈，在实际应用中展现出显著优势。
...
TL;DR: FAConvLSTM：一种用于高分辨率多元地球观测数据的因子化注意力ConvLSTM层，通过轻量级瓶颈和共享深度空间混合改进效率、空间表达能力和物理可解释性，替代传统ConvLSTM2D。
...
Method: 提出FAConvLSTM作为ConvLSTM2D的即插即用替代方案：1）使用轻量级[1×1]瓶颈和共享深度空间混合因子化循环门计算；2）多尺度扩张深度分支和挤压-激励重校准实现跨空间尺度的物理过程建模；3）窥视孔连接增强时间精度；4）轻量级轴向空间注意力机制稀疏应用于时间维度以捕获遥相关尺度依赖；5）专用子空间头生成紧凑时间步嵌入，通过带固定季节性位置编码的时间自注意力进行精炼。

Result: 在多元时空气候数据上的实验表明，FAConvLSTM相比标准ConvLSTM产生更稳定、可解释和鲁棒的潜在表示，同时显著减少计算开销。

Conclusion: FAConvLSTM通过因子化注意力机制有效解决了ConvLSTM在处理地球观测数据时的局限性，在保持循环动态的同时改进了效率、空间表达能力和物理可解释性，为气候数据分析提供了更优的深度学习架构。
...
Method: 提出HOSL混合阶分割学习框架：客户端采用内存高效的零阶梯度估计，消除反向传播和激活存储；服务器端采用一阶优化确保快速收敛和竞争性能。理论证明收敛率取决于客户端模型维度而非完整模型维度。

Result: 在OPT模型（125M和1.3B参数）的6个任务上实验表明：HOSL相比一阶方法减少客户端GPU内存达3.7倍，精度在基线0.20%-4.23%范围内；相比零阶基线性能提升达15.55%，验证了混合策略在边缘设备内存高效训练的有效性。

Conclusion: HOSL通过客户端零阶优化和服务器端一阶优化的混合策略，有效解决了分割学习中内存效率与优化效果之间的权衡，为资源受限边缘设备的大语言模型协作训练提供了实用解决方案。
...
TL;DR: SGD通过非平衡动力学机制偏好平坦解，噪声重塑损失景观为有效势能，瞬态冻结机制最终将动力学困在单一盆地中


<details>
  <summary>Details</summary>
Motivation: 理解SGD为何偏好平坦、泛化性更好的解的动力学起源，建立学习动力学、损失景观几何与泛化之间的统一物理框架

Method: 分析SGD学习动力学，通过数值实验观察轨迹行为，使用可处理的物理模型研究噪声对景观的重塑作用，揭示瞬态冻结机制

Result: 发现SGD噪声将损失景观重塑为偏好平坦解的有效势能，存在瞬态探索阶段，最终通过能量壁垒增长实现冻结机制，增加噪声强度可延迟冻结并促进收敛到更平坦极小值

Conclusion: 为SGD偏好平坦解提供了统一的物理框架，将学习动力学、损失景观几何和泛化联系起来，为设计更有效的优化算法提供了原理指导
...
TL;DR: 提出MGF-RL框架，结合元学习和无梯度进化策略，用于配电网故障恢复，能快速适应新故障场景和可再生能源波动，减少重训练需求。


<details>
  <summary>Details</summary>
Motivation: 极端事件后恢复关键负荷需要自适应控制，但可再生能源不确定性、可调度资源有限和非线性动态使有效恢复困难。传统强化学习泛化能力差，对新故障配置需要大量重训练。

Method: 提出元引导无梯度强化学习(MGF-RL)框架，结合一阶元学习和进化策略，从历史故障经验学习可迁移初始化，无需梯度计算即可快速适应新场景。

Result: 在IEEE 13总线和123总线测试系统中，MGF-RL在可靠性、恢复速度和适应效率方面优于标准RL、基于MAML的元RL和模型预测控制，泛化到未见故障和可再生能源模式，且需要更少微调。

Conclusion: MGF-RL通过元学习和无梯度策略搜索实现高效适应，提供次线性遗憾界理论支持，适用于可再生能源丰富的配电网实时负荷恢复。
...
TL;DR: 轻量级符号推理监督能提升紧凑型自动程序修复模型的修复类型分类性能，通过推理蒸馏方法将大型教师模型的结构化符号推理标签传递给小型学生模型，在不增加模型规模的情况下提高性能。
...
Method: 提出推理蒸馏方法：大型教师模型提供结构化符号推理标签和修复类型标签，这些标签捕获错误的高级因果属性而不依赖自由形式解释。在IntroClass基准上训练基于CodeT5的学生模型，比较仅标签和推理蒸馏两种设置。

Result: 推理监督持续提升宏平均性能，特别是在较少出现的错误类别上，且不增加模型规模或复杂度。正确推理轨迹与正确预测强相关，但不完全决定预测结果。

Conclusion: 符号推理蒸馏是提高轻量级程序修复模型可解释性和鲁棒性的实用方法，表明结构化推理监督能有效增强小型模型对程序结构的理解能力。
...
TL;DR: 该论文澄清了黎曼度量常数缩放对计算几何的影响，区分了变化的量（范数、距离、体积等）和不变的几何对象（联络、测地线、指数映射等），并讨论了在黎曼优化中的意义。


<details>
  <summary>Details</summary>
Motivation: 在计算几何和黎曼优化中，经常通过全局尺度参数对黎曼度量进行常数缩放，但这种操作的影响在实践中并不总是清晰，可能被误认为是曲率、流形结构或坐标表示的变化。作者旨在澄清这种缩放的实际影响，区分哪些量会变化、哪些几何结构保持不变。

Method: 作者提供了一个简洁、自包含的数学分析，系统考察了黎曼度量常数缩放对各种几何和计算对象的影响。通过理论推导，区分了变换量（包括范数、距离、体积元素、梯度大小）和不变几何对象（Levi-Civita联络、测地线、指数映射、对数映射、平行移动）。特别讨论了在黎曼优化中，这种缩放通常可解释为步长的全局重缩放，而非底层几何的修改。

Result: 明确识别了在黎曼度量常数缩放下变化的量：范数、距离、体积元素、梯度大小；以及保持不变的几何对象：Levi-Civita联络、测地线、指数映射、对数映射、平行移动。阐明了在黎曼优化中，这种缩放实际上对应于步长的全局调整，而不改变优化问题的几何本质。

Conclusion: 黎曼度量的常数缩放是一种基本的数学操作，在计算几何中经常使用。理解哪些量会变化、哪些几何结构保持不变至关重要。这种缩放不应被误解为曲率或流形结构的变化，在黎曼优化中，它可以安全地用作步长的全局调整参数，而不影响方法的几何基础。该说明旨在为黎曼计算中引入全局度量尺度参数提供清晰的指导。
...
TL;DR: 本文对对比学习中的后门攻击进行了全面比较性综述，分析了威胁模型、攻击方法、目标领域和现有防御措施，强调了对比学习特有的漏洞，并讨论了安全部署的挑战与未来方向。


<details>
  <summary>Details</summary>
Motivation: 对比学习已成为跨领域自监督表示学习的主流方法，但近期研究表明其易受后门和数据投毒攻击。攻击者可通过操纵预训练数据或模型更新植入恶意行为，这对工业分布式系统的安全部署构成严重威胁，因此需要系统分析该领域的安全漏洞。

Method: 采用系统性文献综述方法，对对比学习中的后门攻击进行全面的比较分析。研究内容包括：威胁模型分类、攻击技术分析、目标领域识别（视觉、多模态、图、联邦学习等）、现有防御机制评估，以及对比学习特有漏洞的深入剖析。

Result: 研究发现对比学习存在特定的安全漏洞，攻击者可通过数据投毒和模型更新操纵在预训练阶段植入后门。不同领域（视觉、多模态、图、联邦学习）的攻击方法和防御策略存在差异。现有防御措施尚不完善，需要针对对比学习特性开发更有效的安全机制。

Conclusion: 对比学习对后门攻击具有显著脆弱性，这对工业分布式系统的安全部署构成严峻挑战。未来研究需要开发针对对比学习特性的防御机制，建立更全面的安全评估框架，并探索在保持模型性能的同时增强安全性的方法。该领域的研究对实际应用的安全部署具有重要意义。
...
TL;DR: 本文提出了一种自反思框架，用于提升图学习模型在具有强伪相关性的Spurious-Motif基准数据集上的可解释性表现。


<details>
  <summary>Details</summary>
Motivation: 现有的可解释图学习方法在Spurious-Motif基准数据集上表现显著较差，该数据集故意设计包含伪相关性，使得模型难以区分真正相关的结构和误导性模式。需要一种方法来提升模型在这种具有挑战性数据集上的可解释性。

Method: 提出了一种自反思框架，可集成到现有的可解释图学习方法中。当现有方法为节点和边生成重要性分数后，该框架将这些预测反馈给原始方法进行第二轮评估。这种迭代过程模仿了大语言模型中使用自反思提示来重新评估先前输出的方法。此外，从图表示学习的角度分析了改进原因，并基于此反馈机制提出了微调训练方法。

Result: 自反思技术能够有效提升模型在具有强伪相关性的Spurious-Motif数据集上的可解释性表现。通过将现有方法的预测结果反馈给模型进行重新评估，可以改善对重要节点和边的识别能力。

Conclusion: 自反思技术不仅适用于大语言模型处理复杂任务，也能有效提升图学习模型在具有伪相关性数据集上的可解释性。提出的自反思框架和微调训练方法为解决Spurious-Motif基准中的挑战提供了有效途径。
...
TL;DR: AVP-Pro：一种基于自适应特征融合和对比学习的两阶段抗病毒肽预测框架，在准确识别AVP和病毒家族分类方面优于现有方法
...
Method: 提出两阶段预测框架：1）构建包含10种描述符的全景特征空间，采用分层融合架构整合自注意力和自适应门控机制；2）使用基于BLOSUM62增强的OHEM对比学习策略提升模型判别能力

Result: 第一阶段通用AVP识别准确率达0.9531，MCC为0.9064，优于现有SOTA方法；第二阶段在小样本条件下实现6个病毒家族和8种特定病毒的准确分类
...
<details>
  <summary>Details</summary>
Motivation: 传统野火风险评估方法经常高估风险，机器学习在气候和地球科学中的应用日益广泛，需要可配置的操作性预测系统

Method: 开发了OpFML（操作性预测与机器学习）管道，这是一个可配置、可适应的周期性预测服务框架

Result: 展示了OpFML管道在每日火灾危险指数预测中的应用能力，并概述了其各种功能特性

Conclusion: OpFML为基于机器学习的操作性预测提供了一个灵活、可配置的解决方案，特别适用于野火危险评估等周期性预测任务
...
### [51] [Soft Bayesian Context Tree Models for Real-Valued Time Series](https://arxiv.org/abs/2601.11079)
*Shota Saito,Yuta Nakahara,Toshiyasu Matsushima*

Main category: cs.LG

TL;DR: 提出Soft-BCT模型，用于实值时间序列，采用软（概率）上下文空间分割而非硬分割，基于变分推断学习算法，在真实数据集上表现与先前BCT相当或更优


<details>
  <summary>Details</summary>
Motivation: 先前实值时间序列的BCT模型采用硬（确定性）上下文空间分割，限制了模型的灵活性和表达能力。需要一种能够处理概率性上下文分割的模型来提高对复杂时间序列的建模能力。

Method: 提出软贝叶斯上下文树模型（Soft-BCT），采用软（概率）分割上下文空间而非硬分割。基于变分推断设计学习算法，通过概率方式处理上下文划分，提高模型灵活性。

Result: 在多个真实世界数据集上，Soft-BCT表现出与先前BCT几乎相同或更优的性能，验证了软分割方法的有效性。

Conclusion: Soft-BCT通过引入软上下文空间分割，扩展了BCT模型在实值时间序列建模中的应用，提供了更灵活的表达能力，并在实际应用中表现出良好性能。
...
### [52] [Differentially Private Subspace Fine-Tuning for Large Language Models](https://arxiv.org/abs/2601.11113)
*Lele Zheng,Xiang Wang,Tao Zhang,Yang Cao,Ke Cheng,Yulong Shen*

Main category: cs.LG

TL;DR: DP-SFT：一种两阶段子空间微调方法，通过仅在低维任务特定子空间中注入差分隐私噪声，在保证隐私的同时显著降低噪声影响，提升大语言模型微调的性能和稳定性。
...
Conclusion: DP-SFT通过将DP噪声限制在任务相关的低维子空间中，有效降低了噪声幅度同时保持正式DP保证，为大语言模型的隐私保护微调提供了高效解决方案。
...
### [53] [Optimized Algorithms for Text Clustering with LLM-Generated Constraints](https://arxiv.org/abs/2601.11118)
*Chaoqi Jia,Weihong Wu,Longkun Guo,Zhigang Lu,Chao Chen,Kok-Leong Ong*

Main category: cs.LG

TL;DR: 提出一种基于大语言模型的约束生成方法，通过生成约束集而非传统成对约束来减少资源消耗，结合定制化约束聚类算法，在保持聚类精度的同时将LLM查询次数减少20倍以上。
...
Method: 1) 提出新颖的约束生成方法，生成约束集而非成对约束，提高查询效率和约束准确性；2) 设计针对LLM生成约束特点的约束聚类算法，引入置信度阈值和惩罚机制处理潜在的不准确约束。

Result: 在五个文本数据集上的实验表明，该方法在聚类精度上达到最先进算法水平，同时将LLM查询次数减少了20倍以上，有效平衡了约束生成成本与整体聚类性能。

Conclusion: 该方法通过高效的约束生成策略和鲁棒的聚类算法，显著降低了基于LLM的约束生成资源消耗，为文本聚类任务提供了一种成本效益高的解决方案。
...
### [54] [Shape-morphing programming of soft materials on complex geometries via neural operator](https://arxiv.org/abs/2601.11126)
*Lu Chen,Gengxiang Chen,Xu Liu,Jingyan Su,Xuhao Lyu,Lihui Wang,Yingguang Li*

Main category: cs.LG

TL;DR: 提出S2NO神经算子，结合谱编码和空间卷积，实现复杂几何体上的高保真形变预测，结合进化算法优化材料分布，实现超分辨率设计
...
Method: 提出谱空间神经算子(S2NO)，集成拉普拉斯特征函数编码和空间卷积，捕捉不规则计算域上的全局和局部形变行为；结合进化算法进行体素级材料分布优化

Result: S2NO能在不规则边界形状、多孔结构和薄壁结构等复杂几何体上实现高保真形变预测；神经算子的离散不变性支持超分辨率材料分布设计，扩展形变设计的多样性和复杂性

Conclusion: S2NO显著提高了复杂形状形变编程的效率和能力，为共形植入、空气动力学形变等高级应用提供了有效工具
...
### [55] [FSL-BDP: Federated Survival Learning with Bayesian Differential Privacy for Credit Risk Modeling](https://arxiv.org/abs/2601.11134)
*Sultan Amed,Tanmay Sen,Sayantan Banerjee*

Main category: cs.LG

TL;DR: 提出联邦生存学习框架FSL-BDP，在保护借款人隐私的同时实现跨机构信用风险建模，解决了传统违约预测中忽略违约时间和数据集中化的问题。


<details>
  <summary>Details</summary>
Motivation: 数据保护法规（如GDPR、CCPA）禁止跨境共享借款人数据，但信用风险模型需要跨机构学习；传统违约预测存在两个局限：二元分类忽略违约时间（将早期违约与晚期违约等同处理），集中式训练违反新兴监管约束。

Method: 提出联邦生存学习框架FSL-BDP，结合贝叶斯差分隐私（BDP），在不集中敏感数据的情况下建模违约时间轨迹；该框架提供贝叶斯（数据依赖）差分隐私保证，同时使机构能够联合学习风险动态。

Result: 在三个真实信用数据集（LendingClub、SBA、Bondora）上的实验表明：联邦学习从根本上改变了隐私机制的相对有效性；虽然经典DP在集中式设置中表现优于贝叶斯DP，但后者从联邦学习中获益显著更多（+7.0% vs +1.4%），在大多数参与客户端中达到接近非私有性能并优于经典DP。
...
### [56] [Context-aware Graph Causality Inference for Few-Shot Molecular Property Prediction](https://arxiv.org/abs/2601.11135)
*Van Thuy Hoang,O-Joun Lee*

Main category: cs.LG

TL;DR: CaMol是一个基于因果推理的上下文感知图学习框架，用于小样本分子属性预测，通过编码化学知识、解耦因果子结构和应用后门调整来提升预测准确性和可解释性。


<details>
  <summary>Details</summary>
Motivation: 现有基于上下文学习的分子属性预测方法存在两个主要局限：1) 未能充分利用与属性因果相关的官能团先验知识；2) 难以识别与属性直接相关的关键子结构。需要从因果推理角度解决小样本场景下的分子属性预测问题。

Method: CaMol采用因果推理框架，假设分子包含决定特定属性的潜在因果结构。主要包含三个核心组件：1) 编码化学知识的上下文图，连接官能团、分子和属性；2) 可学习的原子掩码策略，解耦因果子结构与混淆子结构；3) 分布干预器，通过结合因果子结构和化学基础的混淆因子应用后门调整。

Result: 在多个分子数据集上的实验表明，CaMol在小样本任务中实现了优越的准确性和样本效率，显示出对未见属性的良好泛化能力。发现的因果子结构与官能团的化学知识高度一致，支持模型的可解释性。

Conclusion: CaMol通过因果推理视角有效解决了小样本分子属性预测问题，不仅提升了预测性能，还提供了与化学知识一致的因果子结构解释，为基于Web的蛋白质结构预测和药物发现等服务提供了有力工具。
...
### [57] [Assesing the Viability of Unsupervised Learning with Autoencoders for Predictive Maintenance in Helicopter Engines](https://arxiv.org/abs/2601.11154)
*P. Sánchez,K. Reyes,B. Radu,E. Fernández*

Main category: cs.LG

TL;DR: 比较监督分类与无监督自编码器在直升机发动机故障预测中的性能，监督方法依赖故障标签，自编码器仅需正常数据即可检测异常
...
Method: 比较两种预测性维护策略：1) 监督分类流程，依赖正常和故障行为的标记示例；2) 基于自编码器的无监督异常检测方法，仅使用健康发动机数据学习正常操作模型，将偏差标记为潜在故障
...
### [58] [Clustering High-dimensional Data: Balancing Abstraction and Representation Tutorial at AAAI 2026](https://arxiv.org/abs/2601.11160)
*Claudia Plant,Lena G. M. Bauer,Christian Böhm*

Main category: cs.LG

TL;DR: 该教程探讨聚类算法中抽象与表示之间的平衡，分析从经典K-means到子空间聚类和深度聚类的发展，强调未来需要更自适应地平衡这两者以提高性能、能效和可解释性。


<details>
  <summary>Details</summary>
Motivation: 聚类需要在抽象和表示之间取得平衡：既要抽象掉个体对象的冗余细节，又需要丰富的表示来突出区分不同对象组的关键特征。当前聚类算法在这两个目标之间存在冲突，需要找到更好的平衡方法。

Method: 分析不同聚类算法的抽象-表示权衡：经典K-means采用高抽象（平均化细节）和简单表示（原始空间中的高斯分布）；子空间聚类通过学得两个潜在空间，一个用于聚类相关信息，另一个捕获其他信息；深度聚类通过基于质心和基于密度的聚类损失来强制抽象。

Result: 随着表示表达能力的增强，需要在目标函数中显式强制抽象以确保执行聚类而非单纯表示学习。子空间聚类方法通过分离潜在空间有效平衡抽象和表示，而深度聚类方法通过特定损失函数实现这一平衡。

Conclusion: 未来聚类研究需要更自适应地平衡抽象和表示以提高性能、能效和可解释性。人脑在聚类和相关任务中能自动找到抽象与表示的最佳平衡点，这为算法改进提供了重要启示。
...
### [59] [GMM-COMET: Continual Source-Free Universal Domain Adaptation via a Mean Teacher and Gaussian Mixture Model-Based Pseudo-Labeling](https://arxiv.org/abs/2601.11161)
*Pascal Schlachter,Bin Yang*

Main category: cs.LG

TL;DR: 提出首个持续源自由通用域自适应（continual SF-UniDA）方法GMM-COMET，用于处理多个不同未标记目标域的序列适应问题


<details>
  <summary>Details</summary>
Motivation: 现有SF-UniDA方法仅假设从源域到目标域的单次域偏移，但现实场景中模型需要连续适应多个不同目标域，且源数据不可用、标签空间可能不同

Method: 在先前在线SF-UniDA方法基础上，将高斯混合模型伪标签集成到均值教师框架中，并引入一致性损失以提高长期适应序列的稳定性

Result: GMM-COMET在持续SF-UniDA中提供了首个强基线，在所有评估场景中一致优于仅源域模型，是唯一能持续改进的方法

Conclusion: 该方法成功解决了多目标域序列适应问题，为持续源自由通用域自适应领域建立了首个有效基准
...
### [60] [TimeMar: Multi-Scale Autoregressive Modeling for Unconditional Time Series Generation](https://arxiv.org/abs/2601.11184)
*Xiangyu Xu,Qingsong Zhong,Jilin Hu*

Main category: cs.LG

TL;DR: 提出了一种结构解耦的多尺度时间序列生成框架，通过双路径VQ-VAE分离趋势和季节性成分，采用粗到细的自回归生成方式，在减少参数量的同时生成高质量长期序列。
...
Method: 1) 多尺度离散化编码：将序列编码为多个时间分辨率的离散标记；2) 双路径VQ-VAE：解耦趋势和季节性成分，学习语义一致的潜在表示；3) 粗到细自回归生成：保留层次依赖关系；4) 基于引导的重建策略：利用粗粒度季节性信号作为先验指导细粒度季节性模式重建。

Result: 在六个数据集上的实验表明，该方法比现有方法生成更高质量的时间序列，在显著减少参数量的情况下仍保持强劲性能，并展现出生成高质量长期序列的卓越能力。

Conclusion: 提出的结构解耦多尺度生成框架有效解决了时间序列的结构复杂性，为数据稀缺和隐私挑战提供了有前景的解决方案，同时实现了参数效率和生成质量的双重优势。
...
### [61] [FAQ: Mitigating Quantization Error via Regenerating Calibration Data with Family-Aware Quantization](https://arxiv.org/abs/2601.11200)
*Haiyang Xiao,Weiqing Li,Jinyue Guo,Guochao Jiang,Guohua Liu,Yuewei Zhang*

Main category: cs.LG

TL;DR: FAQ提出了一种基于家族先验知识的校准数据再生框架，通过利用同家族更大LLM生成高保真校准样本，显著提升后训练量化的精度。
...
Method: FAQ框架首先将原始校准样本输入到与目标模型同家族的更大LLM中，利用高度一致的知识体系再生一系列高保真校准数据。这些数据携带思维链推理并符合预期激活分布，随后在专家指导下进行组间竞争以选择最佳样本，最后重新归一化以增强标准PTQ的效果。

Result: 在包括Qwen3-8B在内的多个模型系列上的实验表明，FAQ相比使用原始校准数据的基线方法，将精度损失降低了高达28.5%。

Conclusion: FAQ通过利用同家族LLM的先验知识生成高质量校准数据，有效解决了传统PTQ校准数据不足的问题，展示了其在提升量化精度方面的强大潜力和贡献。
...
TL;DR: SDFLoRA：一种选择性双模块联邦LoRA方法，通过分解全局和本地模块解决联邦学习中LoRA的秩异构问题，实现更好的隐私-效用权衡。


<details>
  <summary>Details</summary>
Motivation: 联邦学习（FL）用于大语言模型（LLMs）时面临秩异构问题，不同客户端使用不同的低秩配置导致LoRA更新聚合存在偏差和不稳定性。现有解决方案强制统一秩或将异构更新对齐到共享子空间，这会过度约束客户端特定语义、限制个性化，并在差分隐私噪声下对本地客户端信息保护较弱。
...
Conclusion: SDFLoRA通过解耦全局和本地模块有效解决了联邦学习中LoRA的秩异构问题，在保持个性化能力的同时实现了鲁棒的聚合和隐私保护，为实际FL部署提供了实用解决方案。
...
### [63] [Operator learning on domain boundary through combining fundamental solution-based artificial data and boundary integral techniques](https://arxiv.org/abs/2601.11222)
*Haochen Wu,Heng Wu,Benzhuo Lu*

Main category: cs.LG

TL;DR: 提出MAD-BNO框架：基于数学人工数据边界神经算子，仅用边界数据学习PDE边界到边界映射，通过边界积分恢复内部解，避免全域采样


<details>
  <summary>Details</summary>
Motivation: 针对已知基本解的线性偏微分方程，传统算子学习方法需要全域采样数据，计算成本高。本文旨在开发仅依赖边界数据的算子学习框架，减少数据需求并保持物理一致性

Method: 结合数学人工数据方法，从目标问题的基本解直接合成训练数据，学习边界到边界映射。训练后通过边界积分公式高效恢复任意位置的内部解，支持多种边界条件和源项

Result: 在二维Laplace、Poisson和Helmholtz方程的基准测试中，MAD-BNO达到或优于现有神经算子方法的精度，同时显著减少训练时间。框架可扩展到三维问题和复杂几何

Conclusion: MAD-BNO为线性PDE提供了一种高效的数据驱动算子学习框架，仅需边界数据即可实现准确求解，具有物理一致性、计算效率高和可扩展性强的优势
...
TL;DR: FEATHer是一种用于边缘设备时间序列预测的超轻量级模型，在严格的计算和内存限制下（仅需400个参数）实现准确的长时预测，在多个基准测试中表现优异。


<details>
  <summary>Details</summary>
Motivation: 随着工业系统向自动化发展，需要在边缘设备（如PLC、微控制器）上进行时间序列预测，这些设备对延迟和内存有严格限制（通常只能容纳几千个参数），传统深度架构在此场景下不实用。

Method: FEATHer采用：1）超轻量级多尺度频率路径分解；2）共享密集时间核（投影-深度卷积-投影结构，无循环或注意力机制）；3）基于频谱特征的频率感知分支门控；4）稀疏周期核（通过周期下采样重建输出以捕捉季节性）。

Result: 在8个基准测试中，FEATHer获得最佳排名，记录了60个第一名结果，平均排名为2.05，证明了在受限边缘硬件上实现可靠长时预测的可行性。

Conclusion: FEATHer展示了在严格计算和内存约束下实现准确长时时间序列预测的可能性，为工业实时推理提供了实用方向，证明了轻量级架构在边缘设备上的有效性。
...
### [65] [Sample-Near-Optimal Agnostic Boosting with Improved Running Time](https://arxiv.org/abs/2601.11265)
*Arthur da Cunha,Miakel Møller Høgsgaard,Andrea Paudice*

Main category: cs.LG

TL;DR: 提出了首个具有接近最优样本复杂度的不可知提升算法，在固定其他参数时运行时间为多项式


<details>
  <summary>Details</summary>
Motivation: 经典提升方法已被充分理解，但在不可知设置（不对数据做任何假设）中理解较少。虽然最近几乎确定了不可知提升的样本复杂度，但已知算法具有指数运行时间

Method: 提出了一种新的不可知提升算法，在固定其他参数时，运行时间相对于样本大小为多项式

Result: 该算法具有接近最优的样本复杂度，是首个在固定其他参数时运行时间为多项式的不可知提升算法

Conclusion: 这项工作填补了不可知提升领域的重要空白，提供了首个具有接近最优样本复杂度且运行时间可行的算法
...
### [66] [Metabolomic Biomarker Discovery for ADHD Diagnosis Using Interpretable Machine Learning](https://arxiv.org/abs/2601.11283)
...
<details>
  <summary>Details</summary>
Motivation: ADHD作为一种常见的神经发育障碍，目前缺乏客观的诊断工具，迫切需要基于生物学的客观诊断框架来推进精准精神病学发展。

Method: 研究整合了52名ADHD患者和46名对照者的尿液靶向代谢组学数据，采用具有嵌入式特征选择功能的Closest Resemblance（CR）分类器进行分析，并与随机森林和K最近邻分类器进行比较。

Result: CR模型性能优于其他分类器，基于14种代谢物（包括多巴胺4-硫酸盐、N-乙酰天冬氨酰谷氨酸和瓜氨酸）达到AUC>0.97。这些代谢物映射到多巴胺能神经传递和氨基酸代谢通路，为ADHD病理生理学提供机制性见解。

Conclusion: 该研究展示了一个结合代谢组学和可解释机器学习的转化框架，CR分类器的透明决策边界和低计算成本支持其整合到靶向代谢组学检测和未来即时诊断平台中，推进ADHD的客观、生物学知情诊断策略。
...
TL;DR: 研究人员整合了多个公开的1型糖尿病数据集，创建了MetaboNet数据集，包含3135名患者和1228患者年的连续血糖监测和胰岛素泵数据，为算法开发提供统一资源。


<details>
  <summary>Details</summary>
Motivation: 当前1型糖尿病算法开发面临的主要挑战是现有数据集分散且缺乏标准化。不同数据集结构差异大，访问和处理耗时，阻碍了数据整合，降低了算法开发的可比性和泛化能力。

Method: 整合多个公开可用的1型糖尿病数据集，要求包含连续血糖监测数据和相应的胰岛素泵给药记录，保留碳水化合物摄入和体力活动等辅助信息。数据集分为完全公开子集（可直接下载）和需数据使用协议限制的子集（通过申请流程访问），为后者提供处理管道以转换为标准化格式。

Result: MetaboNet数据集包含3135名受试者和1228患者年的重叠CGM和胰岛素数据，规模显著大于现有独立基准数据集。数据集涵盖广泛的血糖谱和人口统计学特征，可通过https://metabo-net.org/访问。

Conclusion: 该研究为1型糖尿病研究提供了统一的公共数据集，通过无限制和DUA管理的两种访问途径，能够产生比单个数据集更具泛化性的算法性能。
...
TL;DR: FORESTLLM：结合决策树结构偏置与LLM语义推理的新框架，在少样本表格数据学习中实现SOTA性能，训练时使用LLM作为离线模型设计器，测试时无需LLM推理。


<details>
  <summary>Details</summary>
Motivation: 表格数据在金融、医疗和科学发现等领域用于关键决策，但少样本场景下学习效果不佳。传统树方法依赖统计纯度指标，在有限监督下不稳定易过拟合；直接应用LLM常忽略数据结构导致性能次优。

Method: 提出FORESTLLM框架：1）语义分割准则：LLM基于标记和未标记数据评估候选分割的连贯性，诱导更鲁棒、可泛化的树结构；2）一次性上下文推理机制：LLM将决策路径和支持样本提炼为确定性预测，用语义信息输出替代噪声经验估计。

Result: 在多样化的少样本分类和回归基准测试中，FORESTLLM实现了最先进的性能。

Conclusion: FORESTLLM成功统一了决策树的结构归纳偏置与LLM的语义推理能力，在训练时利用LLM作为离线模型设计器，将丰富的上下文知识编码到轻量级、可解释的森林模型中，测试时无需LLM推理。
...
<details>
  <summary>Details</summary>
Motivation: 扩散语言模型在自然语言处理任务中表现出色，但其在检索增强生成框架中的潜力尚未充分探索。现有方法存在响应语义漂移问题，导致生成内容精度有限，需要解决DLM与RAG结合时的语义对齐问题。

Method: 提出SPREAD框架，引入查询相关性引导的去噪策略。该方法主动引导去噪轨迹，确保生成过程始终锚定查询语义，有效抑制语义漂移。通过系统性测试DLM在RAG框架中的表现，识别出响应语义漂移的根本原因在于去噪策略无法在迭代去噪过程中保持与查询的语义对齐。

Result: 实验结果表明，SPREAD框架显著提升了RAG框架内生成答案的精度，有效缓解了响应语义漂移问题。DLM与RAG结合显示出对上下文信息更强的依赖性，但存在生成精度有限的问题，SPREAD成功解决了这一限制。

Conclusion: SPREAD框架通过查询相关性引导的去噪策略成功解决了扩散语言模型在检索增强生成中的响应语义漂移问题，为DLM与RAG的有效结合提供了创新解决方案，显著提升了生成内容的精度和语义一致性。
...
### [70] [Offline Reinforcement-Learning-Based Power Control for Application-Agnostic Energy Efficiency](https://arxiv.org/abs/2601.11352)
...
<details>
  <summary>Details</summary>
Motivation: 现代计算基础设施设计中能效已成为关键因素，CPU设计已集成功耗调节和感知能力。虽然强化学习适合设计能效控制系统，但在线训练存在挑战：缺乏合适的模拟环境模型、噪声干扰以及在实时系统上部署的可靠性问题。

Method: 采用离线强化学习方法设计自主CPU功耗控制器，利用训练前从任意策略收集的状态转移数据集。结合灰盒方法，使用在线应用无关性能数据（如心跳）和硬件性能计数器，确保科学目标实现且性能下降有限。通过Intel的Running Average Power Limit在实时系统上控制功耗。

Result: 在各种计算密集型和内存密集型基准测试上评估该方法，离线训练的智能体能在可容忍的性能下降成本下显著降低能耗。

Conclusion: 离线强化学习是设计自主CPU功耗控制器的可行替代方案，避免了在线训练的挑战，能够在保证性能可接受下降的前提下有效提高并行应用的运行时能效。
...
### [71] [Factored Value Functions for Graph-Based Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2601.11401)
*Ahmed Rashwan,Keith Briggs,Chris Budd,Lisa Kreusser*

Main category: cs.LG

TL;DR: 提出扩散价值函数(DVF)用于图马尔可夫决策过程，通过奖励在影响图上的扩散为每个智能体分配价值分量，解决了多智能体强化学习中的信用分配问题


<details>
  <summary>Details</summary>
Motivation: 多智能体强化学习中的信用分配问题在大规模具有结构化局部交互的系统中尤为突出。图马尔可夫决策过程通过影响图捕捉这种设置，但标准评论家方法与此结构不匹配：全局价值函数提供较弱的个体学习信号，而现有局部构造在无限时域设置中难以估计且行为不佳

Method: 引入扩散价值函数(DVF)，通过时间折扣和空间衰减在影响图上扩散奖励，为每个智能体分配价值分量。基于DVF提出扩散A2C(DA2C)算法和稀疏消息传递执行器LD-GNN，用于通信成本下的分散算法学习

Result: 在消防基准测试和三个分布式计算任务（向量图着色和两个发射功率优化问题）中，DA2C始终优于局部和全局评论家基线，平均奖励提升高达11%

Conclusion: DVF为图马尔可夫决策过程提供了结构对齐的分解价值函数，解决了信用分配问题，在多种任务中表现出优越性能，为大规模多智能体系统提供了有效的学习框架
...
TL;DR: 该论文提出了一个系统性的压力测试框架，用于评估傅里叶神经算子（FNOs）在五种不同类型偏微分方程下的鲁棒性，揭示了其在分布偏移、长期推演和结构扰动下的脆弱性。


<details>
  <summary>Details</summary>
Motivation: 傅里叶神经算子在求解偏微分方程解映射方面表现出色，但其在分布偏移、长期推演和结构扰动下的鲁棒性尚未得到充分理解。需要系统性地评估FNOs的失效模式，为算子学习的鲁棒性改进提供依据。

Method: 设计了系统性的压力测试框架，包括参数偏移、边界或终端条件变化、分辨率外推与谱分析、迭代推演等控制性测试。在五种不同性质的PDE家族（色散、椭圆、多尺度流体、金融、混沌系统）上进行大规模评估，训练了1000个模型。

Result: 参数或边界条件的分布偏移可使误差增加一个数量级以上；分辨率变化主要将误差集中在高频模式；输入扰动通常不会放大误差，但最坏情况（如局部泊松扰动）仍具挑战性。揭示了谱偏差、复合积分误差和边界条件过拟合等脆弱性。

Conclusion: 研究提供了FNOs失效模式的比较性图谱和可操作的见解，为改进算子学习的鲁棒性奠定了基础。强调了在优化分布内准确性的同时，需要关注模型在分布偏移和极端条件下的表现。
...
TL;DR: 扩散模型集成方法研究：虽然集成能改善分数匹配损失和模型似然，但未能一致提升图像感知质量指标（如FID），在表格数据中某些集成策略表现更优
...
Method: 研究多种集成聚合规则：使用深度集成、蒙特卡洛Dropout等方法，在CIFAR-10和FFHQ数据集上进行实验。同时通过随机森林研究表格数据，并分析分数估计与图像质量之间的联系。

Result: 集成分数通常能改善分数匹配损失和模型似然，但未能一致提升FID等感知质量指标。在表格数据中，发现一种集成策略优于其他方法。理论分析为模型组合技术（如引导）提供了新的见解。

Conclusion: 扩散模型集成在改善统计指标方面有效，但对感知质量指标的提升有限。研究结果为扩散模型集成和模型组合技术提供了理论和实践指导。
...
TL;DR: LRKV是一种通过低秩残差共享KV投影来减少Transformer KV缓存内存的方法，在保持完整token分辨率的同时，通过跨注意力头共享冗余来降低计算需求。


<details>
  <summary>Details</summary>
Motivation: Transformer预训练受到内存和计算资源的限制，其中键值（KV）缓存在训练和自回归解码过程中成为主要瓶颈。需要一种既能减少KV缓存内存占用，又能保持模型性能的注意力机制改进方法。

Method: 提出低秩KV适应（LRKV），对多头注意力进行简单修改：每层使用共享的全秩KV投影，并添加低秩、头特定的残差。这种方法在完全共享和完全独立注意力之间提供连续权衡，是标准多头注意力的直接替代方案。

Result: 在大规模预训练实验中，LRKV相比标准注意力、MQA/GQA和MLA，实现了更快的损失减少、更低的验证困惑度和更强的下游任务性能。在2.5B规模下，LRKV使用约一半的KV缓存就能超越标准注意力性能，达到相同模型质量时训练计算量减少20-25%。

Conclusion: LRKV通过分析算子空间中的注意力头结构，证明其几乎保留了所有功能性头多样性，而更激进的KV共享机制则需要查询专业化补偿。LRKV是内存和计算受限环境下扩展Transformer预训练的一种实用有效的注意力机制。
...
### [75] [Extractive summarization on a CMOS Ising machine](https://arxiv.org/abs/2601.11491)
*Ziqing Zeng,Abhimanyu Kumar,Chris H. Kim,Ulya R. Karpuzcu,Sachin S. Sapatnekar*

Main category: cs.LG

TL;DR: 该论文探索在低功耗CMOS耦合振荡器伊辛机上实现McDonald式抽取式摘要，通过硬件感知的伊辛公式化、随机舍入、迭代优化和分解策略，在CNN/DailyMail数据集上实现了3-4.5倍加速和2-3个数量级的能耗降低。


<details>
  <summary>Details</summary>
Motivation: 现代抽取式摘要系统虽然使用强大的神经模型达到高精度，但通常依赖CPU或GPU基础设施，这些设备能耗高且不适合资源受限环境中的实时推理。本研究旨在探索在低功耗CMOS耦合振荡器伊辛机上实现抽取式摘要的可行性。

Method: 1. 提出硬件感知的伊辛公式化方法，减少局部场和耦合项之间的尺度不平衡，提高对系数量化的鲁棒性；2. 开发完整的抽取式摘要流水线，包括随机舍入和迭代优化以补偿精度损失；3. 采用分解策略将大型摘要问题划分为可在COBI上高效求解的较小伊辛子问题。

Result: 在CNN/DailyMail数据集上，该流水线仅使用有限精度的整数耦合伊辛硬件即可生成高质量摘要。COBI相比暴力方法实现3-4.5倍运行时加速，与软件Tabu搜索相当，同时能耗降低2-3个数量级，同时保持有竞争力的摘要质量。
